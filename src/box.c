...
float box_viou(box a, box b)
{
    ...
    float u = (a.x - b.x) * (a.x - b.x) + (a.y - b.y) * (a.y - b.y);
    float d = u / c;
    float w_gt_pred = b.w / a.w;
    float h_gt_pred = b.h / a.h;
    float ar_loss = (8 / (3*M_PI * M_PI)) * (((atan(w_gt_pred) - (M_PI/4)) * (atan(w_gt_pred) - (M_PI/4)))
     + ((atan(h_gt_pred)- (M_PI/4)) * (atan(h_gt_pred) - (M_PI/4)))+((atan(((b.w*b.h)-(a.w*a.h))/(b.w*b.h)))*(atan(((b.w*b.h)-(a.w*a.h))/(b.w*b.h))))) ;
    float alpha = ar_loss / (1 - iou + ar_loss + 0.000001);
    float viou_term = d + alpha * ar_loss;                   //viou
#ifdef DEBUG_PRINTS
    printf("  c: %f, u: %f, viou_term: %f\n", c, u, viou_term);
#endif
    return iou - viou_term;
}
...
//#endif
	...
	if (iou_loss == VIOU) {
    float w_gt_pred = truth.w / pred.w;
    float h_gt_pred = truth.h / pred.h;
    float ar_loss = (8 / (3*M_PI * M_PI)) * (((atan(w_gt_pred) - (M_PI/4)) * (atan(w_gt_pred) - (M_PI/4)))
     + ((atan(h_gt_pred)- (M_PI/4)) * (atan(h_gt_pred) - (M_PI/4)))+((atan(((truth.w*truth.h)-(pred.w*pred.h))/(truth.w*truth.h)))
     *(atan(((truth.w*truth.h)-(pred.w*pred.h))/(truth.w*truth.h))))) ;
	float alpha = ar_loss / (1 - I/U + ar_loss + 0.000001);
	float ar_dw=(16/(3*M_PI*M_PI))*(((atan(w_gt_pred) - (M_PI/4)) *(truth.w/(truth.w*truth.w + pred.w *pred.w)))+
	((atan(((truth.w*truth.h)-(pred.w*pred.h))/(truth.w*truth.h)))*((pred.h*truth.h*truth.w)/((truth.w*truth.h*truth.w*truth.h)
	+((truth.w*truth.h-pred.w*pred.h)*(truth.w*truth.h-pred.w*pred.h))))));
    float ar_dh=(16/(3*M_PI*M_PI))*(((atan(h_gt_pred) - (M_PI/4)) * (truth.h/(truth.h*truth.h + pred.h * pred.h)))+
    ((atan(((truth.w*truth.h)-(pred.w*pred.h))/(truth.w*truth.h)))*((pred.w*truth.h*truth.w)/((truth.w*truth.h*truth.w*truth.h)
	+((truth.w*truth.h-pred.w*pred.h)*(truth.w*truth.h-pred.w*pred.h))))));
    if (C > 0) {
    // dar*
        p_dx += (2*(truth.x-pred.x)*C-(2*Cw*dCw_dx+2*Ch*dCh_dx)*S) / (C * C);
